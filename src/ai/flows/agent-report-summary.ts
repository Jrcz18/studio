...
export const AgentReportSummaryInputSchema = z.object({
  ...
  totalBookings: z.number().describe('The total number of bookings secured by the agent.'),
  totalRevenueGenerated: z.number().describe('The total revenue generated from the agent\'s bookings.'),
  totalCommission: z.number().describe('The total commission earned by the agent.'),
});
...
export async function generateAgentReportSummary(
  input: AgentReportSummaryInput
): Promise<AgentReportSummaryOutput> {
