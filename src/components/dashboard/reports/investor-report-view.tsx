...
export function InvestorReportView({ report }: { report: any }) {
  ...
  const handlePrint = () => {
    const printContent = document.getElementById('investor-report-content')?.innerHTML;
    const printWindow = window.open('', '_blank', 'height=800,width=800');
    if (printWindow) {
        const fileName = `Investor_Report_${report.investor.name.replace(/ /g, '_')}_${report.month}_${report.year}`;
